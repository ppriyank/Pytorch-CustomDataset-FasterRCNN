...
def read_all(directory, filename):
    ...
    return json.loads(string)["element"]
...
# 		labels.append(elements["category"])
# labels = set(labels)
...
def process(data):
	...
	for e in data:
		...
		for elements in e["objects"]:
			...
			min_x = bounding_box['minimum']['r']
			min_y = bounding_box['minimum']['c']
			max_x = bounding_box['maximum']['r']
			max_y = bounding_box['maximum']['c']
...
train_data = process(data_train)
test_data = process(data_test)

with open("TRAIN_images.json", 'w') as output:
	output.write( str(train_data[1]).replace("'", "\"") )


with open("TRAIN_objects.json", 'w') as output:
	output.write( str(train_data[0]).replace("'", "\"")  )


with open("TEST_images.json", 'w') as output:
	output.write(  str(test_data[1]).replace("'", "\"")  )


with open("TEST_objects.json", 'w') as output:
	output.write(  str(test_data[0]).replace("'", "\"") )
