...
########################################################################################################################################################################################################
# Train on 1 example  (overfit to check if its working fine )
########################################################################################################################################################################################################
...
rpm = RPM(anchor_sizes , anchor_ratios, valid_anchors, config.rev_label_map, rpn_max_overlap=0.7 , rpn_min_overlap= 0.3)

dataset_train =  Dataset(data_folder='./', rpm=rpm, split='TRAIN', std_scaling=std_scaling, image_resize_size= (height, width),  debug= False)
train_loader = DataLoader(
    dataset_train, shuffle=True,  collate_fn=collate_fn, 
    batch_size=2, num_workers=4, pin_memory=pin_memory, drop_last=True)
